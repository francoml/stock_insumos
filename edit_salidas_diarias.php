...
if (isset($_POST['salidas'])) {
	...
	if (empty($errors)) {
		...
		if ($result && $db->affected_rows() === 1) {
			...
			redirect('salidas_diarias.php', false);
		} else {
			$session->msg('d', 'No se pudo modificar.');
			redirect('edit_salidas_diarias.php?id=' . $salida['id'], false);
		}
	} else {
...
<form method="post" action="edit_salidas_diarias.php?id=<?php echo (int)$salida['id'] ?>">
