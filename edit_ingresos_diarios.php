...
if (isset($_POST['remito'])) {
	...
	if (empty($errors)) {
		...
		if ($result && $db->affected_rows() === 1) {
			...
			redirect('ingresos_diarios.php', false);
		} else {
			$session->msg('d', 'No se pudo modificar.');
			redirect('edit_ingresos_diarios.php?id=' . $remito['id'], false);
		}
	} else {
...
<form method="post" action="edit_ingresos_diarios.php?id=<?php echo (int)$remito['id'] ?>">
